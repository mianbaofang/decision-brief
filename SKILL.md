...
| `--base-url` | 否 | 后端 API 地址，默认 `http://127.0.0.1:8010` |
...
| `--weather-base-url` | 否 | 高德天气接口地址，如 `https://restapi.amap.com/v3/weather/weatherInfo` |
...
```bash
# 1. 用自动模式分析一个问题（需配置 LLM Key，或先在 Web 设置中主动开启 Demo）
...
1. **请求级覆盖（CLI flag / ChatRequest 字段）**：`--api-key`、`--llm-model`、
   `--llm-base-url`、`--weather-key`（高德 Key）、`--weather-base-url`、`--weather-city`，
   每次调用临时传入，不落盘。
...
- 未配置 LLM Key 且未主动开启 Demo 时，`/api/chat` 返回 402。用户可在 Web 提示中
  选择“体验 Demo”；示例结果会附 `source: "mock"` 标识。
- 未同时配置天气 Key 和 Base URL 时，nature 模式返回基于关键词的 mock 自然意象简报
...
- nature 模式时 `brief` 为 `null`，改返回 `nature` 自然意象简报。除了解读文字，
  还包含位置、气温、湿度、月相、近期天气和 `signals.weights` 等参考值；接口拿不到的字段为空。
...
- `--action config-api` 返回脱敏配置
  `{ llm: { model, baseUrl, hasKey }, weather: { city, baseUrl, hasKey, hasBaseUrl, hasAppsecret }, hasLlm, hasWeather }`。
  `hasWeather` 只有在 Key 和 Base URL 都已配置时才为 `true`；`hasAppsecret` 是兼容旧前端的别名，等价于 `hasKey`。
...
python main.py
# 或热重载：uvicorn main:app --reload --port 8010
```

接口文档见 `http://127.0.0.1:8010/docs`，详见 `backend/README.md`。

## 前端 UI

后端启动后会自动挂载 `frontend/` 目录的静态资源（仿 HTML 版完整 UI）。用户可通过浏览器访问：

- **首页（决策面板）**：`http://127.0.0.1:8010/`
